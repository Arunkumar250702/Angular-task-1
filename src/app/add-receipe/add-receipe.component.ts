...
export class AddReceipeComponent {
  ...
  contentAssetUrl: string = ''; //  store uploaded img URL
  ...
  uploadImage() {
    ...
    this.recipeService.uploadImage(this.selectedFile).subscribe(
      (response) => {
        if (response && response.data[0].storage_name) {
          this.contentAssetUrl = "https://eknologiunggul.sgp1.digitaloceanspaces.com/" + response.data[0].storage_name;
      ...
      (error) => {
        alert(`Failed to upload image: ${error.statusText}`);
      }
    );
  }
    ...
    if (this.isEditMode && this.recipeId) {
      this.recipeService.updateRecipe(this.recipeId, recipePayload).subscribe(
        () => {
          ...
          this.generatePDF(); //pdf generetae
          this.router.navigate(['/receipe']);
        },
        (error) => {
          alert(`Failed to update recipe: ${error.statusText}`);
        }
      );
    } else {
      this.recipeService.createRecipe(recipePayload).subscribe(
        ...
        (error) => {
          alert(`Failed to save recipe: ${error.statusText}`);
        }
      );
    }
  }
...
generatePDF() {
  ...
  if (this.contentAssetUrl) {
    html2canvas(document.getElementById('previewImage') as HTMLImageElement).then(canvas => {
      const imgData = canvas.toDataURL('image/png');
      doc.addImage(imgData, 'PNG', 10, 90, 50, 50);
